...
public class RISKMap implements Serializable {
     ...
     * @param getAllNeighbors   true to get all neighbors, false to only get
     *                          territories not owned by player
    ...
    public Territory findNearestNeighbor(Territory startTerritory, Player player) {
        ...
        // Create a priority queue to store the territories by their distance from the
        // start territory
     ...
     * @return the distance between them, if there is no such a path return
     *         Integer.MAX_VALUE
     */
        ...
        while (!visited.contains(dest) && !pq.isEmpty()) {
            ...
            if (edges != null) {
                for (Edge edge : edges) {
                    ...
                    if (hasSameOwner && ((dest.getOwner().getAlliancePlayer() == null && !source.getOwner().equals(neighbor.getOwner()))
                            || (!dest.getOwner().equals(neighbor.getOwner())
                                    && (dest.getOwner().getAlliancePlayer() != null
                                            && !dest.getOwner().getAlliancePlayer().equals(neighbor.getOwner()))))) {
